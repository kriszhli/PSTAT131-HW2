...
The histogram of age illustrates a right-skewed distribution. (The red dashed line is the median.)
...
I chose an 80/20 split.

```{r}
set.seed(11)
library(caTools)
ab$spl <- sample.split(ab$age,SplitRatio=0.80)
train <- subset(ab, ab$spl==TRUE)
test <- subset(ab, ab$spl==FALSE)
```
...
```{r}
library(fastDummies)
ab_dum <- dummy_cols(ab, select_columns = "type")
center <- function(x) {
    scale(x, scale = FALSE)
}
ab_dum_cen <- ab_dum
ab_dum_cen[ ,c(2:10)] <- center(ab_dum[ ,c(2:10)])

ab_dum_cen$longest_shell_x_diameter <- ab_dum_cen$longest_shell * ab_dum_cen$diameter
ab_dum_cen$shucked_weight_x_shell_weight <- ab_dum_cen$shucked_weight * ab_dum_cen$shell_weight
```
...
```{r}
lreg <- lm(ab_dum_cen$age~ab_dum_cen$longest_shell+ab_dum_cen$diameter+ab_dum_cen$height+ab_dum_cen$whole_weight+ab_dum_cen$shucked_weight+ab_dum_cen$viscera_weight+ab_dum_cen$shell_weight+ab_dum_cen$type_F+ab_dum_cen$type_I+ab_dum_cen$type_M+ab_dum_cen$longest_shell_x_diameter+ab_dum_cen$shucked_weight_x_shell_weight)
summary(lreg)
```
